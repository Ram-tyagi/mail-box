...
const Allmails = () => {
  ...
  async function hideStar(item) {
    ...
    try {
      let response = await axios.put(`https://mailbox-database-2d55d-default-rtdb.firebaseio.com/${mail}/data.json`,
      {
        obj,
      },
      {
  ...
  async function deleteMail(id) {
    ...
    try {
      let response = await axios.delete(
        `https://mailbox-database-2d55d-default-rtdb.firebaseio.com/${mail}/data/${id}.json`
      );
      if (response.status === 200) {
        console.log(response.data);
      } else {
