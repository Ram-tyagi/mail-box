...
const Allmails = () => {
  ...
  async function hideStar(item) {
    ...
    try {
      let link;
      if (recievemsg) {
        link = `https://mailbox-database-2d55d-default-rtdb.firebaseio.com/${mail}/send/${item.id}.json`;
      } else {
  ...
  async function deleteMail(id) {
    ...
    let link;
    if (recievemsg) {
      link = `https://mailbox-database-2d55d-default-rtdb.firebaseio.com/${mail}/send/${id}.json`;
    } else {
